...
# Helper to clean up the data headers so the scripts can match serial numbers
def prepare_files_for_comparison(mts_file, machine_file):
    try:
        for file in [mts_file, machine_file]:
            if os.path.exists(file):
                df = pd.read_csv(file)
                # Standardize 'Serial No' column for matching
...
st.write("Upload your raw data to run the automated sorting and statistical analysis.")
...
if uploaded_file is not None:
    ...
    # Save the file locally so subprocesses can access it
    ...
    if st.button("🚀 Start Analysis"):
        try:
            ...
            # File definitions (Must match your script outputs)
            ...
            with st.status("Processing...", expanded=True) as status:
                ...
                # Check for Normalised file before running IR_test12
                if os.path.exists(normalised_out):
                    ...
                    st.error(f"Error: {normalised_out} was not created.")
                    st.stop()

                st.write("🧹 Standardizing headers...")
                prepare_files_for_comparison(mts_ng_out, machine_out)

                # Final Check before Venn Diagram generation
                if os.path.exists(mts_ng_out) and os.path.exists(machine_out):
            ...
            if images:
                ...
                st.warning("No Venn diagrams found. Verify if matching failures exist.")

            st.info(f"Process complete. Files are stored in: {output_folder}")
            
        except subprocess.CalledProcessError as e:
            st.error("A background script failed to complete.")
